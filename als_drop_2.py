...
def main(spark, train_file,val_file, model_file):
    df_train = spark.read.parquet(train_file).filter("count >2")
    df_val = spark.read.parquet(val_file).filter("count >2")
    ...
    best_model = best.fit(df_train)
    best_model.save(model_file)
if __name__ == "__main__":

    # Create the spark session object
    spark = SparkSession.builder.appName('drop_2').config("spark.memory.fraction", 0.9).config("spark.executor.memory", "4g").config("spark.driver.memory", "4g").getOrCreate()
